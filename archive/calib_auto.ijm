...
// NEED TO INPUT FRAME NUMBER VARIABLES, Z NUMBER VARIABLES
	for (j=0; j<30; j++) {; 				// this loop takes a multidimensional zt stack and splits it into stacks of z, WOULD WORK IF STARTED ON 0 BUT DONT WANT THAT NAMING CONV				
	...
	file_list = getFileList(stack_directory); 		// retrieves names of all separated z stacks
		...
		run("TIFF Virtual Stack...", "open=" + file_location);
			...
			setSlice(i);
			pixel_shift = 2* (i - 1);
			canvas_height = 410 - (2* (i - 1));			// calculating the realignment amounts
			...
			run("Canvas Size...", "width=410 height=" +(410 - (2* (i-1)))+ " position=Top-Center"); //need way of measuring canvas dimensions and auto parsing that into script
			rename(i);
			run("Canvas Size...", "width=410 height=410 position=Bottom-Center"); //resizing to original dimensions for stack rebuilding
			};
...
run("Stack to Hyperstack...", "order=xyczt(default) channels=1 slices=60 frames=30 display=Color"); // 	NEED TO INPUT VARIABLES FOR Z AND T 
rename("mega")