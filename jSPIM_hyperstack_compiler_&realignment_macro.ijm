...
getDimensions(width, height, channels, slices, frames);
	...
	file_list = getFileList(stack_directory); 							// retrieves names of all separated z stacks
		...
		run("TIFF Virtual Stack...", "open=" + file_location );
			...
			setSlice(i);
			pixel_shift = 4* (i - 1);
			canvas_height = height - (4* (i - 1));																	// calculating the realignment amounts
			run("Make Substack...", "slices=" + i); 															 	//select sequential slices to edit, rather than just realigning the first slice repeatedly
			run("Canvas Size...", "width=" + width + " height=" +( height - (4* (i-1)))+ " position=Top-Center"); 	//need way of measuring canvas dimensions and auto parsing that into script
			rename(i);
